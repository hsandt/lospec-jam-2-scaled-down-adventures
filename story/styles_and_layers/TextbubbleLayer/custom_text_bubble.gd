...
func _process(_delta:float) -> void:
	...
	# LOCAL HOTFIX by hsandt to avoid Inference on Variant warning treated as error (when parsing addons)
...
func _resize_bubble(content_size:Vector2, popup:=false) -> void:
	...
	# CUSTOM bubble has no shader-based box size (just normal Control rect size)
